...
$keys=[];
for ($for = 1; $for <= $soni; $for++) {
$keys[]=["text"=>"$more[$for]","callback_data"=>"tahichk-".$kat."-".$more[$for]];
...
for ($for = 1; $for <= $soni; $for++) {
$title=str_replace("\n","",$more[$for]);
$keys[]=["text"=>"$more[$for]","callback_data"=>"deleteich-".$kat."-".$more[$for]];
$keysboard2 = array_chunk($keys, $ibyson);
...
$ta = substr_count($ichida,"\n");
$keys[]=["text"=>"$more[$for] ($ta)->","callback_data"=>"delochir-".$kat."-".$more[$for]];
$keysboard2 = array_chunk($keys, $ibyson);
...
$key[]=["text"=>"$for","callback_data"=>"deletexiz-".$for."-".$kat."-".$roy];
}
...
$ichida = file_get_contents("sozlamalar/bot/$kat/".$more[$for]."/xizmatbolim.txt");
$ta = substr_count($ichida,"\n");
$keys[]=["text"=>"$more[$for] ($ta)->","callback_data"=>"tanxiz-".$kat."-".$more[$for]];
$keysboard2 = array_chunk($keys, $ibyson);
...
for ($for = 1; $for <= $soni; $for++) {
$text .= "<b>$for.</b> ".$ids[$for]."\n";
$key[]=["text"=>"$for","callback_data"=>"tahrirla-".$kat."-".$roy."-".$for];
}
$keysboard2 = array_chunk($key, $xizson);
...
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>"📝 Nomni tahrirlash",'callback_data'=>"nomi-xizmat-".$kat."-".$roy."-".$xiz]],
[['text'=>"📝 IDni tahrirlash",'callback_data'=>"editBot-id-".$kat."-".$roy."-".$xiz],['text'=>"📝 Narxni tahrirlash",'callback_data'=>"editBot-narx-".$kat."-".$roy."-".$xiz]],
[['text'=>"📝 Min. tahrirlash",'callback_data'=>"editBot-min-".$kat."-".$roy."-".$xiz],['text'=>"📝 Max. tahrirlash",'callback_data'=>"editBot-max-".$kat."-".$roy."-".$xiz]],
...
$ichida = file_get_contents("sozlamalar/bot/$kat/".$more[$for]."/xizmatbolim.txt");
$ta = substr_count($ichida,"\n");
$keys[]=["text"=>"$more[$for] ->","callback_data"=>"addxiz-".$more[$for]."-".$kat];
$keysboard2 = array_chunk($keys, $ibyson);
$keysboard2[] = [['text'=>"◀️ Orqaga",'callback_data'=>"xizmatlar"]];
$AdBot = json_encode([
'inline_keyboard'=>$keysboard2,
...
$ichida = file_get_contents("sozlamalar/bot/$kat/".$more[$for]."/xizmatbolim.txt");
$ta = substr_count($ichida,"\n");
$keys[]=["text"=>"$more[$for] ($ta)","callback_data"=>"ichki-".$more[$for]."-".$kat];
$keysboard2 = array_chunk($keys, $ibyson);
...
$key[]=["text"=>"$for","callback_data"=>"byxiz-".$ids[$for]."-".$kat."-".$roy];
}
$keysboard2 = array_chunk($key, $xizson);
...
$kategoriya = $ex[2];
$ichki2bolim = $ex[1];
$xizmat = file_get_contents("sozlamalar/bot/$kategoriya/$royxat/$ichki2bolim/xizmat.txt");
$narx=file_get_contents("sozlamalar/bot/$kategoriya/$royxat/$ichki2bolim/narx.txt");
$id=file_get_contents("sozlamalar/bot/$kategoriya/$royxat/$ichki2bolim/id.txt");
$min=file_get_contents("sozlamalar/bot/$kategoriya/$royxat/$ichki2bolim/min.txt");
$max=file_get_contents("sozlamalar/bot/$kategoriya/$royxat/$ichki2bolim/max.txt");
bot('editMessageText',[
'chat_id'=>$ccid,
'message_id'=>$cmid,
...
'chat_id'=>$ccid,
'message_id'=>$cmid,
'text'=>"<b>👥 Foydalanuvchilar soni: $lich ta</b>",
'parse_mode'=>"html",
'reply_markup'=>json_encode([
'inline_keyboard'=>[
